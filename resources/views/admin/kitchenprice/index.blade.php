...
@section('title')
<title>All Kitchen Price | Admin</title>
@endsection
        ...
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                    <h4 class="mb-sm-0 font-size-18">All Kitchen Price</h4>
                    @if(Auth::user()->hasPermissionTo('product_add'))
                    <div class="page-title-right">
                        <a href="{{route('admin.kitchen-price.create')}}" class="btn btn-primary waves-effect waves-light"><i class="fas fa-plus"></i> Add New Kitchen Price</a>
                    </div>
                    @endif
                </div>
            </div>
                ...
                <div class="card border">
                    <div class="card-body">
                        ...
                        >
                            ...
                            <tbody>
                            	@if($kitchenprices->count())
                            	@foreach($kitchenprices as $key => $kitchenprice)
                                <tr>
                                    <td>
                                        {{$key+1}}
                                    </td>
                                    ...
                                    <td><div class="content m-0 p-0">
                                        <p class="m-0"><b>{{$kitchenprice->user->name}}</b></p>
                                        {{-- <p class="m-0"><b>
                                            @if($product->category)
                                            [{{$product->category->name}}]
                                            @endif
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="{{ route('admin.kitchen-price.edit', $kitchenprice->product_id) }}" class="btn btn-soft-info btn-sm waves-effect waves-light"><i class="bx bx-pencil font-size-16"></i></a>
                                        @endif
                                        @if(Auth::user()->hasPermissionTo('product_delete'))
                                        <a href="javascript:void(0);" class="btn btn-soft-danger btn-sm waves-effect waves-light sa-delete" title="Delete Product" data-id="{{ $kitchenprice->id }}" data-link="/admin/product/destroy/"><i class="bx bx-trash font-size-16"></i></a>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
