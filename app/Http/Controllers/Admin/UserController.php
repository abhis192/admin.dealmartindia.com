...
class UserController extends Controller
{
    ...
    public function kitchenPriceIndex()
    {
        $customers = User::whereRoleId(3)->whereNull('deleted_at')->get();
        $products=Product::all();
        $kitchenprices = KitchenPrice::get();
        return view('admin.kitchenprice.index', compact('customers','products','kitchenprices'));
    }


    public function kitchenPriceCreate()
    {
        $user = User::whereRoleId(2)->get();
        $types = Type::all();
        $categories = Category::all();
        $products = Product::all();

        return view('admin.kitchenprice.create', compact('user','types','categories','products'));
    }


    public function kitchenPricestore(Request $request)
    {
        $request->validate([
            'product_id' => 'required',
        ]);
        ...
        // dd($request->all());
        // done || Ab aram sa foreach loop chala do save ho jayega


        // $userMapping = UserMapping::whereUserId($id)->first();
        if(!empty($request->prod)) {
            foreach ($request->prod as $key => $val) {
                $priceData['kitchen_id'] = $request->kitchen_id;
                $priceData['product_id'] = $request->product_id;
                $priceData['qty_type'] = $request->prod[$key]['qty_type'];
                $priceData['qty_weight'] = $request->prod[$key]['qty_weight'];
                $priceData['price'] = $request->prod[$key]['price'];
                $kitchenProductPrice = KitchenPrice::create($priceData);
                }
            }
        ...
        // KitchenPrice::create($data);
        return redirect('/admin/kitchen-price')->with('success','Kitchen Price created successfully.');
    }


    public function kitchenpriceedit($id)
    {
        ...
        $products = Product::all();
        // $tags = ProductTag::all();
        return view('admin.kitchenprice.edit', compact('product','categories','types','user','products'));
    }


     //kitchenPrice-Config

     public function kitchenPriceConfig()
     {
         $products=Product::all();
         return view('admin.kitchenpriceconfig.index', compact('products'));
     }

     public function kitchenPriceStoreConfig(Request $request) {
        $requestData = $request->all();

        foreach ($requestData['products'] as $productId => $productData) {
            foreach ($productData['prices'] as $priceData) {
                $existingRecord = ConfigKitchenPrice::where('product_id', $productId)
                    ->where('qty_type', $priceData['qty_type'])
                    ->where('qty_weight', $priceData['qty_weight'])
                    ->first();

                if ($existingRecord) {
                    $existingRecord->price = $priceData['price'];
                    $existingRecord->save();
                } else {
                    $configKitchenPrice = new ConfigKitchenPrice();
                    $configKitchenPrice->product_id = $productId;
                    $configKitchenPrice->qty_type = $priceData['qty_type'];
                    $configKitchenPrice->qty_weight = $priceData['qty_weight'];
                    $configKitchenPrice->price = $priceData['price'];
                    $configKitchenPrice->save();
                }
            }
        }

        return redirect('admin/kitchenPrice-Config')->with('success','ConfigKitchenPrice data saved successfully');
    }
}
